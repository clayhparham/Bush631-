...
head(lastdayintrade)
n <- length(lastdayintrade)
resultsbet <- rep(NA, n)
for (i in 1:n) {
  resultsbet <- lastdayintrade$PriceD - lastdayintrade$PriceR
  lastdayintrade$resultsbet <- resultsbet[i]
  sign(resultsbet)
}
sign(resultsbet)
...
for (i in 1:n) {
  resultspoll <-lastdayintrade$Obama - lastdayintrade$McCain
  lastdayintrade$resultspoll <- resultspoll[i]
  sign(resultspoll)
}
sign(resultspoll)

#intresults08$DaysToElection <- as.Date("2008-11-04") -   as.Date(intresults08$day)
# make a dummy variable in the dataframe that looks to see if the results of the bet are equal to the results of the polls. If they are return 1, if not return 0
```
