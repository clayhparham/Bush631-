...
```{r}
corruption <- read.csv("/Users/clayparham/Documents/Bush631-/Assignment Nine/corruption.csv")
```
 -------------------------------- ----------------------------------------------------------
 Name                             Description
 -------------------------------- ----------------------------------------------------------
...
The forcing variable is the population size, since there is a cutoff point with the size of the population where they get money. The authors cannot simply compare the treated and non-treated since the it's not a guaranteed random distribution, however, by using an RD design, we are able to have a cutoff and see how the cutoff impacts the data in two separate experiments. It almost forces a randomness into the design. One weakness of the RD is that if there is another factor that also is demarcated along the forcing variable line, that may skew the data in some way.
...
It seems that the more money given slightly increases the amount of corruption in both broad and narrow definitions. However, the more population the less the corruption, although the population category increases the level of corruption.
```{r}
lm(broad ~ fpm, data = corruption)
lm(broad ~ fpm + log(pop), data = corruption)
lm(broad ~ fpm + log(pop) + pop_cat, data = corruption)

lm(narrow ~ fpm, data = corruption)
lm(narrow ~ fpm + log(pop), data = corruption)
lm(narrow ~ fpm + log(pop) + pop_cat, data = corruption)
...
```{r}
head(corruption)
large <- subset(corruption, subset = (pop <= (10188+500) & pop >= 10188))
small <- subset(corruption, subset = (pop >= (10188-500) & pop <= 10188))
small.range <- c(min(small$pop), max(small$pop))
large.range <- c(min(large$pop), max(large$pop))
large.lm <- lm(fpm ~ pop, data = large)
small.lm <- lm(fpm ~ pop, data = small)
large.line <- predict(large.lm, newdata = data.frame(pop = large.range))
small.line <- predict(small.lm)

plot(corruption$pop, corruption$fpm, 
     xlim = c(9600, 11000), ylim = c(10, 40),
     abline(v = 10188, lty = "dashed"),
     lines(large.range, large.line), col = "red")
     #lines(lm(small$fpm ~ small$pop), col = "blue")
